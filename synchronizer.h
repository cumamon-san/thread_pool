...
public:
  ...
  template <typename T> class unique_access : utils::noncopyable_t {
  public:
    unique_access(T *value, std::shared_mutex &mtx)
  ...
  template <typename T> class shared_access : utils::noncopyable_t {
  public:
    shared_access(T *value, std::shared_mutex &mtx)
        : value_(value), lock_(mtx) {}
    T *operator->() { return value_; }
    T &operator*() { return *value_; }
